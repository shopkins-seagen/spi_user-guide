...
#. If the group is not available in the dropdown list, send a request to SPI to register the group in the application. To do that provide the group name and path to the .bat file 
   ...
   that location (`See SPI identity creation <spi.html>`__ ) . The idenities are also members of SAS_Users and SAS_ClinProg_GXP and will have access to the infrastructure outside of the SS folder, effectively impersonating
